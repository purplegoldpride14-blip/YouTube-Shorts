...
def render_clip_segment(args):
    ...
    if s["dur"] <= VIDEO_CLIP_SEC + 0.05:
        cmd = ["ffmpeg", "-v", "error", "-y", "-i", src, "-t", str(s["dur"]),
              "-vf", vf, "-an", "-c:v", "libx264", "-crf", str(VIDEO_CRF),
              "-preset", VIDEO_PRESET, dst]
    else:
        cmd = ["ffmpeg", "-v", "error", "-y", "-stream_loop", "-1", "-i", src,
              "-t", str(s["dur"]), "-vf", vf, "-an", "-c:v", "libx264",
              "-crf", str(VIDEO_CRF), "-preset", VIDEO_PRESET, dst]
    run(cmd)
    return dst


def prep_music_bed(project_dir, total_dur):
    """narration == 'none': find audio/music.* by hand, loudnorm to the bed
    level, and loop/trim it to the video's total duration."""
    ...
    filt = f"loudnorm=I={MUSIC_BED_LUFS}:TP={TARGET_TP}:LRA={TARGET_LRA}"
...
def main():
    ...
    if asset_mode == "clips":
        ...
        with ThreadPoolExecutor(max_workers=a.jobs) as ex:
            segs = list(ex.map(render_clip_segment, [(s, frames_dir, seg_dir) for s in scenes]))
